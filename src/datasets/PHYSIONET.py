...
class Physionet(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        val_tensors = [tensor(data['x_train_full'])]
        test_tensors = [tensor(data['x_train_full'])]

        # missing and mask, 1 is an evidence for missing data

        train_tensors += [tensor(data['x_train_miss']), (tensor(data['m_train_miss']) == 0)]

        val_tensors += [tensor(data['x_train_miss']),  (tensor(data['m_train_miss']) == 0)]

        test_tensors += [tensor(data['x_train_miss']), (tensor(data['m_train_miss']) == 0)]

        train_tensors.append(tensor(data['y_train']))
        val_tensors.append(tensor(data['y_train']))
        test_tensors.append(tensor(data['y_train']))
